				...
				<table cellpadding="0" cellspacing="0" border="0" width="100%" style="" class="mod-comment-body">
					<tr>
						<td align="center" colspan="2">
							<script src="resources/js/modules/ImageGallery/functions.js">void(1);</script>
...
<xsl:template match="IMAGE-GALLERY-THUMBNAIL">
	<div>
		<xsl:attribute name="style">float: left; margin-right: 10px; margin-bottom: 10px; height:<xsl:value-of select="@MAX-HEIGHT-THUMBNAIL + 70"/>px;</xsl:attribute>
		<table border="0" cellspacing="0" class="mod-comment-body">
			<xsl:attribute name="style">height:<xsl:value-of select="@MAX-HEIGHT-THUMBNAIL + 50"/>px;</xsl:attribute>
			<tr>
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%" class="mod-search" style="margin: 10px 0; height: 40px;">
		<tr>
			<td align="left" style="padding-left: 10px;">
				<form name="search" action="" style="margin: 0;" method="GET">	
					<input type="hidden" name="view" value="thumbnailList"/>
					<xsl:value-of select="@TITLE"/>
					<input name="q" class="mod-input" style="width: 205px; margin: 0 10px;">
						<xsl:attribute name="value"><xsl:value-of select="@VALUE" disable-output-escaping="yes"/></xsl:attribute>
					</input>
					<xsl:value-of select="@SELECT-TITLE" disable-output-escaping="yes"/>
					<span style="padding-left:10px; padding-right:10px;"><xsl:apply-templates select="INPUT"/></span>
					<input type="submit" value="Search" class="mod-search-button" style="height: 25px;">
						<xsl:attribute name="value">&#160;&#160;&#160;<xsl:value-of select="@BUTTON-TITLE" disable-output-escaping="yes"/>&#160;&#160;&#160;</xsl:attribute>
					</input>
				</form>
			</td>	
		</tr>
